...
- `phone_verified_at` + EnquiryDraft's `$phoneVerified` are the OTP seam, and the homepage Quick Enquiry box now uses it: QuickEnquiryController runs App\Modules\CreditScore\Actions\RequestMobileOtp/VerifyMobileOtp in front of RecordEnquiry and passes true. Any other form is wired the same way — nothing else changes. The loan-page form is still unverified.
