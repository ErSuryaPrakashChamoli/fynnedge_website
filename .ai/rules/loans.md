...
Loan product views (`show`, `show-flexi-hybrid`, `landing-page`) use `<article itemscope itemtype="https://schema.org/Service">` as the outer wrapper (NOT FinancialProduct — FynnEdge is a DSA, see the Service rule in layouts-views.md, pinned by EntityGraphTest) (was `<section>`; nothing in app.css targets either element, and show-flexi-hybrid keeps its `id="flexi-hybrid-details"` jump-link anchor). Sections carry `data-ai-context="..."` labels. The FAQ accordion deliberately has NO microdata — the FAQPage JSON-LD already declares those Question/Answer entities and marking them up twice would double-declare them.
